namespace LUAnityExample
{
	...
	public static class L2U
	{
		...
		public static object RequireLua( string luaScriptPathName )
		{
			byte[] luaCode = null;
			object[] results = null;
			...
			try
			{
				TextAsset ta = Resources.Load( string.Format( "LuaScripts/{0}.lua", luaScriptPathName ) ) as TextAsset;
				if( ta != null )
				{
					luaCode = ta.bytes;
				}
				...
				{
					Debug.LogError( "Load Lua script file failed! (" + luaScriptPathName + ")" );
				}
			}
			catch
			{
				Debug.LogWarning( "Lua script file does NOT exist! (" + luaScriptPathName + ")" );
			}
			...
			if( luaCode != null )
			{
				try
				{
					results = lua.DoString( luaCode, luaScriptPathName );
				}
				catch( Exception e )
				{
					Debug.LogError( string.Format( "Failed to do RequireLua(\"{0}\"): {1} {2}", luaScriptPathName, e.Source, e.Message ) );
				}
			}
		...
		public static GameObject FindGameObject( string goName, GameObject parentGO, bool mustExist )
		{
			...
			{
				Debug.LogError( string.Format( "Cannot find gameobject with name {0} under gameObject {1}", goName, parentGO ) );
			}
